...
class PressModel:
    ...
    offsets: Union[dict, list, np.ndarray] = {"C1-20": 106.798502}):
        ...
        if type(slopes) == dict and enabled_bunkers:
            self.slopes = np.array([slopes.get(bunker, None) for bunker in enabled_bunkers])
            self.offsets = np.array([offsets.get(bunker, None) for bunker in enabled_bunkers]) 
            ...
            raise ValueError("Parameters must be of types dict, list or ndarray. They must all be of the same type.")

    def get_pressing_time(self, current_time, time_prev_pressing_done, bunker_idx, n_bales):
        """
        ...
        if current_time > time_prev_pressing_done or not self.slopes[bunker_idx]:
            ...
            return None
        return current_time + self.slopes[bunker_idx] * n_bales + self.offsets[bunker_idx]